/**
 * The store, as statements the worker can apply to itself.
 *
 * Every one of these creates only if absent, so applying the whole list is
 * always safe and always idempotent. That is what lets the worker bring its
 * own database up to date on a cold start: adding a feature that needs a new
 * table becomes a deploy rather than a deploy plus a migration somebody has to
 * remember to run.
 *
 * `schema.sql` is generated from this file for anyone who would rather apply
 * it by hand, and the offline checks fail if the two drift apart:
 *
...
/** The same statements as a file, for applying by hand. */
...
// Running this file prints the SQL, which is how schema.sql is kept in step.
